...
export const checkEmail: RequestHandler = (req, res) => {
  ...
  if (user) {
    return res.json({ exists: true, needsPassword: !!user.passwordHash, role: user.role });
  }
...
export const register: RequestHandler = (req, res) => {
  ...
  if (!emailLower.endsWith("@gmail.com")) return res.status(400).json({ error: "Only Gmail addresses allowed" });
  ...
  const user = { id: `user_${Math.random().toString(36).slice(2, 9)}`, name: name || email.split("@")[0], email: emailLower, role: normalizedRole, createdAt: new Date().toISOString() } as any;
  db.users = db.users || [];
  db.users.push(user);
...
export const login: RequestHandler = (req, res) => {
  ...
  if (!emailLower.endsWith("@gmail.com")) return res.status(400).json({ error: "Clients must use Gmail" });
  ...
  if (user.passwordHash !== hash(password)) return res.status(403).json({ error: "Invalid credentials" });

...
export default function StaffInvoices() {
  ...
  async function submit(e: React.FormEvent){
    ...
    const res = await fetch('/api/admin/invoices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({clientId,amount: Math.round(Number(amount))})});
    const data = await res.json();
    if(res.ok){ setMessage('Invoice created'); setAmount(''); }
      ...
      <section className="container mx-auto px-4 py-10">
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          <form onSubmit={submit} className="rounded-lg border p-6">
            ...
            <div className="mt-4">
              <button type="submit" className="rounded-md bg-primary px-4 py-2 text-primary-foreground">Create Invoice</button>
            </div>
          ...
          <div className="rounded-lg border p-6">
            ...
            <div className="mt-2 text-sm text-muted-foreground">Invoices are stored in-memory on the server.</div>
          </div>
        </div>
      </section>
    </StaffDashboardLayout>

...
export default function StaffManagement() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setMessage(null);
    const res = await fetch("/api/admin/staff", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, email, role, password }) });
      ...
      <section className="container mx-auto px-4 py-10">
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          <form onSubmit={submit} className="rounded-lg border p-6">
            ...
            <div className="mt-4">
              <button type="submit" className="rounded-md bg-primary px-4 py-2 text-primary-foreground">Create Staff</button>
            </div>
          ...
          <div className="rounded-lg border p-6">
            ...
            <div className="mt-2 text-sm text-muted-foreground">Staff list is managed in memory (server). Use Supabase integration to persist.</div>
          </div>
        </div>
      </section>
    </StaffDashboardLayout>
